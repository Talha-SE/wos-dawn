...
router.post('/chat', requireAuth, async (req: AuthRequest, res) => {
  ...
  try {
    ...
    const body = {
      ...
      stream: false,
      instructions: 'you are a smart and sassy chatting model, your name is kitsuya',
    };
...
// Proxy tool file content (e.g., images generated by tools) by file ID
router.get('/tool-file/:id', requireAuth, async (req: AuthRequest, res) => {
  const apiKey = env.MISTRAL_API_KEY
  if (!apiKey) return res.status(500).json({ message: 'MISTRAL_API_KEY not configured on server' })
  ...
  try {
    const upstream = await fetch(`https://api.mistral.ai/v1/files/${id}/content`, {
    ...
    res.status(500).json({ message: e?.message || 'Failed to fetch tool file' })
  }
})
